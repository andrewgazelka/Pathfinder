...
public class PathfinderJNI {
    ...
    public static Trajectory generateTrajectory(Waypoint[] waypoints, Trajectory.Config config) {
        return new Trajectory(generateTrajectory(waypoints, config.fit, config.sample_count, config.dt, config.max_velocity, config.max_acceleration, config.max_jerk));
    }
